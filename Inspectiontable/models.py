...
Description:  
Author: limaochao
Date: 2020-11-20 20:47:27
LastEditTime: 2020-11-20 20:49:41
'''
...
class Airconditioning(models.Model):
    Airconditioning_list = models.ForeignKey(
        AirconditioningList, on_delete=models.DO_NOTHING)
    # AirconditioningListname = models.CharField('空调列表', max_length=100)
    Airconditioning_temperature = models.IntegerField(
        '空调温度', default=21, null=True)
    Open_state = models.BooleanField(
        '开启状态', default=True, null=True)
    Abnormal_noise = models.BooleanField(
        '异常噪音', default=False, null=True)
    Airconditioning_Remarks = models.TextField('空调备注', null=True)
    Airconditioning_Create_Data = models.DateTimeField(
